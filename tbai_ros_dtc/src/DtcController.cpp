...
DtcController::DtcController(const std::shared_ptr<tbai::StateSubscriber> &stateSubscriber)
    ...
    horizon_ = 1.0;  // TODO(lnotspotl): Load from config
    mpcRate_ = 30;   // TODO(lnotspotl): Load from config
    pastAction_ = vector_t().setZero(12);

    // Initialize quadruped interface first (needed for kinematics)
    ...
    auto kinematicsSharedPtr = std::shared_ptr<switched_model::KinematicsModelBase<ocs2::scalar_t>>(
...
std::vector<vector3_t> DtcController::getDesiredFeetVelocities(scalar_t currentTime, scalar_t dt) {
    ...
    auto jointAnglesOcs2 = switched_model::getJointPositions(optimizedState);
    auto jointVelocitiesOcs2 =
        switched_model::getJointVelocities(optimizedInput);  // TODO(lnotspotl): Bug, this should be input
    std::vector<vector3_t> feetVelocities(4);
    for (int legidx = 0; legidx < 4; ++legidx) {
        auto footVelocity =
            kin.footVelocityInOriginFrame(legidx, basePoseOcs2, baseTwistOcs2, jointAnglesOcs2, jointVelocitiesOcs2);
