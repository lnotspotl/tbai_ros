...
void SwingPlanningVisualizer::preSolverRun(scalar_t initTime, scalar_t finalTime, const vector_t &currentState,
    ...
    for (int leg = 0; leg < NUM_CONTACT_POINTS; leg++) {
        ...
        swingTrajectoriesMessage.markers.reserve(
            swingTrajectoryPlannerPtr_->getTargetTrajectories().timeTrajectory.size() + 1);  // lower bound
        ...
        for (const auto &t : swingTrajectoryPlannerPtr_->getTargetTrajectories().timeTrajectory) {
            ...
            swingTrajectoriesMessage.markers.push_back(
                ocs2::getArrowAtPointMsg(arrowScale * node.velocity, node.position, feetColorMap_[leg]));
