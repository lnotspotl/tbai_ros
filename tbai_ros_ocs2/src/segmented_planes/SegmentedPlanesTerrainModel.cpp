...
TerrainPlane SegmentedPlanesTerrainModel::getLocalTerrainAtPositionInWorldAlongGravity(
    ...
    const auto projection =
        getBestPlanarRegionAtPositionInWorld(positionInWorld, planarTerrain_.planarRegions, std::move(penaltyFunction));
...
ConvexTerrain SegmentedPlanesTerrainModel::getConvexTerrainAtPositionInWorld(
    ...
    for (const auto &point : convexRegion) {
        convexTerrain.boundary.emplace_back(
            point.x() - projection.positionInTerrainFrame.x(),
...
void SegmentedPlanesTerrainModel::createSignedDistanceBetween(const Eigen::Vector3d &minCoordinates,
    ...
    const auto minXY = grid_map::lookup::projectToMapWithMargin(
        planarTerrain_.gridMap, grid_map::Position(minCoordinates.x(), minCoordinates.y()));
    const auto maxXY = grid_map::lookup::projectToMapWithMargin(
        planarTerrain_.gridMap, grid_map::Position(maxCoordinates.x(), maxCoordinates.y()));
    ...
    if (success) {
        signedDistanceField_ = std::make_unique<SegmentedPlanesSignedDistanceField>(
            subMap, elevationLayerName, minCoordinates.z(), maxCoordinates.z());
    } else {
...
vector3_t SegmentedPlanesTerrainModel::getHighestObstacleAlongLine(const vector3_t &position1InWorld,
    ...
    const auto result = grid_map::lookup::maxValueBetweenLocations({position1InWorld.x(), position1InWorld.y()},
...
std::vector<vector2_t> SegmentedPlanesTerrainModel::getHeightProfileAlongLine(const vector3_t &position1InWorld,
    ...
    if (diffSquareNorm > (resolution * resolution)) {  // norm(p2-p1)_XY > resolution
        const auto pointsOnLine = grid_map::lookup::valuesBetweenLocations({position1InWorld.x(), position1InWorld.y()},
                                                                           {position2InWorld.x(), position2InWorld.y()},
                                                                           planarTerrain_.gridMap, *elevationData_);
