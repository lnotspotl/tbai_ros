...
void MRT_ROS_Interface::launchNodes(ros::NodeHandle& nodeHandle) {
  ...
  mpcObservationPublisher_ = nodeHandle.advertise<tbai_ros_ocs2::mpc_observation>(topicPrefix_ + "_mpc_observation", 1);

  // target trajectories publisher
  targetTrajectoriesPublisher_ = nodeHandle.advertise<tbai_ros_ocs2::mpc_target_trajectories>(topicPrefix_ + "_mpc_target", 1);
