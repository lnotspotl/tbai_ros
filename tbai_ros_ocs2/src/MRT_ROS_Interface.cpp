...
/******************************************************************************************************/
/******************************************************************************************************/
void MRT_ROS_Interface::setTargetTrajectories(const TargetTrajectories &targetTrajectories) {
    targetTrajectoriesPublisher_.publish(ros_msg_conversions::createTargetTrajectoriesMsg(targetTrajectories));
}

/******************************************************************************************************/
/******************************************************************************************************/
/******************************************************************************************************/
void MRT_ROS_Interface::setModeSchedule(const ModeSchedule &modeSchedule) {
    modeSchedulePublisher_.publish(ros_msg_conversions::createModeScheduleMsg(modeSchedule));
}
...
/******************************************************************************************************/
/******************************************************************************************************/
void MRT_ROS_Interface::readPolicyMsg(const tbai_ros_ocs2::mpc_flattened_controller &msg, CommandData &commandData,
    ...
    if (msg.stateTrajectory.size() != N && msg.inputTrajectory.size() != N) {
        throw std::runtime_error(
            "[MRT_ROS_Interface::readPolicyMsg] state and input trajectories must have same length!");
    }
    if (msg.data.size() != N) {
    ...
    switch (msg.controllerType) {
        case tbai_ros_ocs2::mpc_flattened_controller::CONTROLLER_FEEDFORWARD: {
        ...
        case tbai_ros_ocs2::mpc_flattened_controller::CONTROLLER_LINEAR: {
            auto controller =
                LinearController::unFlatten(stateDim, inputDim, primalSolution.timeTrajectory_, controllerDataPtrArray);
...
/******************************************************************************************************/
/******************************************************************************************************/
void MRT_ROS_Interface::mpcPolicyCallback(const tbai_ros_ocs2::mpc_flattened_controller::ConstPtr &msg) {
...
void MRT_ROS_Interface::shutdownNodes() {
    ...
    modeSchedulePublisher_.shutdown();
}
...
void MRT_ROS_Interface::launchNodes(ros::NodeHandle &nodeHandle) {
    ...
    mpcObservationPublisher_ =
        nodeHandle.advertise<tbai_ros_ocs2::mpc_observation>(topicPrefix_ + "_mpc_observation", 1);

    // target trajectories publisher
    targetTrajectoriesPublisher_ =
        nodeHandle.advertise<tbai_ros_ocs2::mpc_target_trajectories>(topicPrefix_ + "_mpc_target", 1);

    // mode schedule publisher
    modeSchedulePublisher_ = nodeHandle.advertise<tbai_ros_ocs2::mode_schedule>(topicPrefix_ + "_mpc_mode_schedule", 1);

    // policy subscriber
    auto ops = ros::SubscribeOptions::create<tbai_ros_ocs2::mpc_flattened_controller>(
    ...
    mpcResetServiceClient_ = nodeHandle.serviceClient<tbai_ros_ocs2::reset>(topicPrefix_ + "_mpc_reset");
