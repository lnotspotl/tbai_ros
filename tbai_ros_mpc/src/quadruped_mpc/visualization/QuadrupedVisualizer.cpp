...
void QuadrupedVisualizer::publishTrajectory(const std::vector<ocs2::SystemObservation> &system_observation_array,
                                            double speed) {
    for (size_t k = 0; k < system_observation_array.size() - 1; k++) {
        ...
        double publishDuration =
            ocs2::timedExecutionInSeconds([&]() { publishObservation(ros::Time::now(), system_observation_array[k]); });
...
void QuadrupedVisualizer::publishCartesianMarkers(ros::Time timeStamp, const contact_flag_t &contactFlags,
    ...
    for (int i = 0; i < NUM_CONTACT_POINTS; ++i) {
        markerArray.markers.emplace_back(ocs2::getFootMarker(feetPosition[i], contactFlags[i], feetColorMap_[i],
                                                             footMarkerDiameter_, footAlphaWhenLifted_));
        markerArray.markers.emplace_back(
            ocs2::getForceMarker(feetForce[i], feetPosition[i], contactFlags[i], ocs2::Color::green, forceScale_));
    }

    // Center of pressure
    markerArray.markers.emplace_back(ocs2::getCenterOfPressureMarker(feetForce.begin(), feetForce.end(),
                                                                     feetPosition.begin(), contactFlags.begin(),
                                                                     ocs2::Color::green, copMarkerDiameter_));

    // Support polygon
    markerArray.markers.emplace_back(ocs2::getSupportPolygonMarker(
        feetPosition.begin(), feetPosition.end(), contactFlags.begin(), ocs2::Color::black, supportPolygonLineWidth_));
...
void QuadrupedVisualizer::publishDesiredTrajectory(ros::Time timeStamp,
    ...
    for (size_t k = 0; k < stateTrajSize; ++k) {
        ...
        velArray.markers.emplace_back(
            ocs2::getArrowAtPointMsg(o_baseVel / velScale_, o_basePosition, ocs2::Color::blue));
        angVelArray.markers.emplace_back(
            ocs2::getArrowAtPointMsg(o_baseAngVel / velScale_, o_basePosition, ocs2::Color::green));
        ...
        for (size_t i = 0; i < NUM_CONTACT_POINTS; i++) {
            ...
            const vector3_t b_footVelocity =
                kinematicModelPtr_->footVelocityRelativeToBaseInBaseFrame(i, qJoints, qVelJoints) +
            ...
            feetVelArray[i].markers.emplace_back(
                ocs2::getArrowAtPointMsg(o_footVelocity / velScale_, o_footPosition, feetColorMap_[i]));
        }
    }

    // Headers
    auto baseLineMsg = ocs2::getLineMsg(std::move(desiredBasePositionMsg), ocs2::Color::green, trajectoryLineWidth_);
    ...
    for (size_t i = 0; i < NUM_CONTACT_POINTS; i++) {
        auto footLineMsg =
            ocs2::getLineMsg(std::move(desiredFeetPositionMsgs[i]), feetColorMap_[i], trajectoryLineWidth_);
        footLineMsg.header = ocs2::getHeaderMsg(frameId_, timeStamp);
        footLineMsg.id = 0;
        costDesiredFeetPositionPublishers_[i].publish(footLineMsg);
        ocs2::assignHeader(feetVelArray[i].markers.begin(), feetVelArray[i].markers.end(),
...
void QuadrupedVisualizer::publishOptimizedStateTrajectory(ros::Time timeStamp, const scalar_array_t &mpcTimeTrajectory,
    ...
    markerArray.markers.reserve(NUM_CONTACT_POINTS +
                                2);  // 1 trajectory per foot + 1 for the future footholds + 1 for the Base trajectory
    for (int i = 0; i < NUM_CONTACT_POINTS; i++) {
        markerArray.markers.emplace_back(
            ocs2::getLineMsg(std::move(feetMsgs[i]), feetColorMap_[i], trajectoryLineWidth_));
        markerArray.markers.back().ns = "EE Trajectories";
    }
    markerArray.markers.emplace_back(
        ocs2::getLineMsg(std::move(mpcBasePositionMsgs), ocs2::Color::red, trajectoryLineWidth_));
    ...
    for (int event = 0; event < eventTimes.size(); ++event) {
        if (tStart < eventTimes[event] &&
            eventTimes[event] < tEnd) {  // Only publish future footholds within the optimized horizon
            ...
            const vector_t postEventState =
                ocs2::LinearInterpolation::interpolate(eventTimes[event], mpcTimeTrajectory, mpcStateTrajectory);
            ...
            for (int i = 0; i < NUM_CONTACT_POINTS; i++) {
                if (!preEventContactFlags[i] &&
                    postEventContactFlags[i]) {  // If a foot lands, a marker is added at that location.
...
void QuadrupedVisualizer::publishEndEffectorPoses(
    ros::Time timeStamp, const feet_array_t<vector3_t> &feetPositions,
