...
void QuadrupedVisualizer::publishCartesianMarkers(ros::Time timeStamp, const contact_flag_t& contactFlags,
  ...
  for (int i = 0; i < NUM_CONTACT_POINTS; ++i) {
    markerArray.markers.emplace_back(
        ocs2::getFootMarker(feetPosition[i], contactFlags[i], feetColorMap_[i], footMarkerDiameter_, footAlphaWhenLifted_));
    markerArray.markers.emplace_back(ocs2::getForceMarker(feetForce[i], feetPosition[i], contactFlags[i], ocs2::Color::green, forceScale_));
  }

  // Center of pressure
  markerArray.markers.emplace_back(ocs2::getCenterOfPressureMarker(feetForce.begin(), feetForce.end(), feetPosition.begin(), contactFlags.begin(),
                                                             ocs2::Color::green, copMarkerDiameter_));

  // Support polygon
  markerArray.markers.emplace_back(ocs2::getSupportPolygonMarker(feetPosition.begin(), feetPosition.end(), contactFlags.begin(),
...
void QuadrupedVisualizer::publishDesiredTrajectory(ros::Time timeStamp, const ocs2::TargetTrajectories& targetTrajectories) const {
  ...
  for (size_t k = 0; k < stateTrajSize; ++k) {
    ...
    velArray.markers.emplace_back(ocs2::getArrowAtPointMsg(o_baseVel / velScale_, o_basePosition, ocs2::Color::blue));
    angVelArray.markers.emplace_back(ocs2::getArrowAtPointMsg(o_baseAngVel / velScale_, o_basePosition, ocs2::Color::green));
    ...
    for (size_t i = 0; i < NUM_CONTACT_POINTS; i++) {
      ...
      feetVelArray[i].markers.emplace_back(ocs2::getArrowAtPointMsg(o_footVelocity / velScale_, o_footPosition, feetColorMap_[i]));
    }
  }
  ...
  for (size_t i = 0; i < NUM_CONTACT_POINTS; i++) {
    auto footLineMsg = ocs2::getLineMsg(std::move(desiredFeetPositionMsgs[i]), feetColorMap_[i], trajectoryLineWidth_);
...
void QuadrupedVisualizer::publishOptimizedStateTrajectory(ros::Time timeStamp, const scalar_array_t& mpcTimeTrajectory,
  ...
  for (int i = 0; i < NUM_CONTACT_POINTS; i++) {
    markerArray.markers.emplace_back(ocs2::getLineMsg(std::move(feetMsgs[i]), feetColorMap_[i], trajectoryLineWidth_));
    markerArray.markers.back().ns = "EE Trajectories";
  }
  markerArray.markers.emplace_back(ocs2::getLineMsg(std::move(mpcBasePositionMsgs), ocs2::Color::red, trajectoryLineWidth_));
