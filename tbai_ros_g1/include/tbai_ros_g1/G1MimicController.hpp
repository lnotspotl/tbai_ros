...
constexpr int MIMIC_TOTAL_OBS_SIZE = MIMIC_OBS_MOTION_CMD + MIMIC_OBS_ANCHOR_ORI +
                                     MIMIC_OBS_BASE_ANG_VEL + MIMIC_OBS_JOINT_POS_REL +
                                     MIMIC_OBS_JOINT_VEL_REL + MIMIC_OBS_LAST_ACTION;  // 154

class G1MimicController : public tbai::Controller {
   public:
    G1MimicController(const std::shared_ptr<tbai::StateSubscriber> &stateSubscriberPtr,
                      const std::string &policyPath,
                      const std::string &motionFilePath,
                      float motionFps = 60.0f,
   ...
   private:
    ...
    quaternion_t computeTorsoOrientation(const quaternion_t &rootQuat,
